...
-(void)prepareLayout{
    ...
    for (NSUInteger i = 0; i < itemsCount; i++){
        ...
        float itemWidthAtIndexPath = [self.delegate collectionView:self.collectionView widthForItemAtIndexPath:indexPath];

        
        itemWidth = [self columnWidth] * itemWidthAtIndexPath;
        itemHeight = [self columnWidth] * itemRelativeHeight;


        //  Set column height
        for (NSUInteger i = columnIndex; i < (columnIndex + itemWidthAtIndexPath); i++) {
            _columns[i] = @(yOffset + itemHeight);
        }
